...
J=-1.0; g=2.5; chi=10; d=2; delta=0.01; N=30000;
...
for step in range(0, N):
    ...
    # Apply U
    theta = np.tensordot(theta,U,axes=([1,2],[0,1])) # 1,2 corresponds to j and k in the notes for theta tilde
    ...
    E_iTEBD = -np.log(np.sum(theta**2))/delta/2
    if np.abs(E_iTEBD-E0_exact)*100/np.abs(E0_exact)<10**(-3):
        print step
        break
